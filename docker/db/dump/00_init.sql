...
CREATE TABLE `agency_rules` (
 `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
 `agency_id` int(10) unsigned NOT NULL COMMENT 'ID агентства',
 `rules` varchar(2000) NOT NULL DEFAULT '[]' COMMENT 'Правила',
 `text` varchar(500) NOT NULL DEFAULT 'Внимание менеджеру' COMMENT 'Текст сообщения для менеджера',
 `is_active` tinyint(1) NOT NULL DEFAULT 0 COMMENT 'Активное правило',
 PRIMARY KEY (`id`),
 FOREIGN KEY (`agency_id`) REFERENCES agencies(`id`)
) ENGINE=InnoDB AUTO_INCREMENT=0 DEFAULT CHARSET=utf8mb4 COMMENT='Правила агентств';